...
class ViewBudgetDetailsUI:
    ...
    def initialize(self):
        ...
        label = ttk.Label(master=self._frame, text=f"Details of {self._budget.name}")
        label.grid(row=0, column=0, padx=5, pady=8, sticky=constants.W)

        income = ttk.Label(master=self._frame, text=f"Income: {self._budget.income}")
        ...
        bills = ttk.Label(master=self._frame, text=f"Bills: {self._budget.bills}")
        bills.grid(row=3, column=0, padx=5, pady=8, sticky=constants.W)

        hobbies = ttk.Label(master=self._frame, text=f"Hobbies: {self._budget.hobbies}")
        hobbies.grid(row=4, column=0, padx=5, pady=8, sticky=constants.W)

        misc = ttk.Label(master=self._frame, text=f"Miscellaneous: {self._budget.misc}")
        misc.grid(row=5, column=0, padx=5, pady=8, sticky=constants.W)

        difference = ttk.Label(master=self._frame, text=f"Money left over: {budgeting_service.show_difference(self._budget)}")
